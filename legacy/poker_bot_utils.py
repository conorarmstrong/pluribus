...
class AdvancedAbstraction:
    ...
    def _compute_preflop_buckets(self) -> Dict[Tuple[int, int], int]:
        ...
        # Simplified bucketing - (high_rank, low_rank, suited)
        # Premium hands
        premium = [(12, 12, False), (11, 11, False), (10, 10, False),
                   (12, 11, False), (12, 11, True)]  # AA, KK, QQ, AKo, AKs
        for hand in premium:
            buckets[hand] = 0

        # Strong hands
        strong = [(9, 9, False), (8, 8, False),
                  (12, 10, False), (12, 10, True),   # AQo, AQs
                  (12, 9, True)]                     # AJs
        for hand in strong:
            buckets[hand] = 1

        # Medium pairs and broadway
        medium = [(7, 7, False), (6, 6, False),
                  (11, 10, False), (11, 10, True),   # KQo, KQs
                  (10, 9, True)]                     # JTs
        for hand in medium:
            buckets[hand] = 2
    ...
    def _normalize_hole_cards(self, hole_cards: Tuple[int, int]) -> Tuple[int, int, bool]:
        """Normalize hole cards by rank, preserving suited/offsuit distinction"""
        r1, r2 = hole_cards[0] % 13, hole_cards[1] % 13
        s1, s2 = hole_cards[0] // 13, hole_cards[1] // 13
        suited = (s1 == s2)
        return (max(r1, r2), min(r1, r2), suited)
    
    def _estimate_equity(self, hole_cards: Tuple[int, int],
                        board: List[int], num_opponents: int = 1,
                        samples: int = 100) -> float:
        """Estimate hand equity via Monte Carlo, supporting multi-way pots"""
        wins = 0.0
        used_cards = set(list(hole_cards) + list(board))
        remaining = [c for c in range(52) if c not in used_cards]
        needed = num_opponents * 2 + max(0, 5 - len(board))

        if len(remaining) < needed:
            return 0.5  # Not enough cards to simulate; assume breakeven

        for _ in range(samples):
            sample = list(np.random.choice(remaining, size=needed, replace=False))
            new_board = list(board) + sample[num_opponents * 2:]
            my_str = self.evaluator.evaluate_hand(list(hole_cards), new_board)

            beat_all = True
            for i in range(num_opponents):
                opp_cards = sample[i * 2:(i + 1) * 2]
                opp_str = self.evaluator.evaluate_hand(opp_cards, new_board)
                if opp_str > my_str:
                    beat_all = False
                    break
                elif opp_str == my_str:
...
class ExploitabilityCalculator:
    ...
    def calculate_best_response(self, strategy_profile: Dict, player: int) -> float:
        ...
        A full implementation requires traversing self.game_tree to find the
        optimal counter-strategy. Returning 0.0 until the game tree interface
        is wired up, so calculate_exploitability stays numeric.
        """
        return 0.0
